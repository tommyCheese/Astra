...
for y in request["y"]:
    ...
    elif request["backend"] == "seaborn" and kind in {"histogram", "box", "violin"}:
    ...
    elif kind == "bar": axis.bar(frame[request["x"]], frame[y], label=y)
    elif kind == "scatter": axis.scatter(frame[request["x"]], frame[y], label=y)
    else: axis.plot(frame[request["x"]], frame[y], label=y)
axis.set_title(request.get("title", ""))
if len(request["y"]) > 1: axis.legend()
figure.tight_layout()
for output in request["outputs"]:
    if output in {"png", "svg"}: figure.savefig(f"/output/chart.{output}")
