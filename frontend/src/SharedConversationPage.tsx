...
export function SharedConversationPage({ token }: { token: string }) {
  ...
  return <main className="shared-page">
    ...
    {missing ? <section className="shared-empty"><h1>分享链接不可用</h1><p>该链接不存在、已停止分享，或原对话已被删除。</p></section> : conversation ? <article className="shared-conversation"><h1>{conversation.title}</h1><p className="shared-meta">只读快照 · 更新于 {new Date(conversation.updated_at).toLocaleString()}</p>{conversation.messages.map((message, index) => message.role === 'process' ? <details className="shared-process" key={`process-${index}`}><summary><span className="shared-process-icon">✦</span><strong>思考过程</strong><small>{message.items.length} 个步骤</small></summary><div className="shared-process-timeline">{message.items.map((item, itemIndex) => <div className={`shared-process-item ${item.kind} ${item.status}`} key={`${item.kind}-${itemIndex}`}><span className="shared-process-dot" /><div><strong>{item.title}</strong>{item.detail && <p>{item.detail}</p>}<small>{item.status === 'failed' ? '失败' : item.status === 'cancelled' ? '已终止' : '已完成'}</small></div></div>)}</div></details> : <section className={`shared-message ${message.role}`} key={`${message.role}-${index}`}><span>{message.role === 'user' ? '用户' : 'Astra'}</span><ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown></section>)}</article> : <section className="shared-empty"><p>正在加载共享对话…</p></section>}
  </main>;
}
