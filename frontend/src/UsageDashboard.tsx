...
export function UsageDashboard({ taskId, runId, onClose }: { taskId?: string; runId?: string; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="usage-dashboard" role="dialog" aria-modal="true" aria-label={t('用量统计')} onMouseDown={(event) => event.stopPropagation()}>
      <header><div><span>{t('持久化统计')}</span><h2>{t('用量看板')}</h2><p>{t('数据来自数据库与模型供应商，不使用前端估算。')}</p></div><button className="close-button" type="button" aria-label={t('关闭用量统计')} onClick={onClose}>×</button></header>
      ...
      {loading ? <div className="usage-state">{t('正在读取持久化用量…')}</div> : error ? <div className="usage-state error"><strong>{t('加载失败')}</strong><span>{error}</span><button type="button" onClick={() => setReload((value) => value + 1)}>{t('重试')}</button></div> : !data || (!data.overview.model_invocations && !data.overview.tool_calls && !data.overview.agent_turns) ? <div className="usage-state"><strong>{t('所选范围暂无用量记录')}</strong><span>{t('完成一次任务后，模型、工具与产物用量会在此显示。')}</span></div> : <>
        ...
        <section className={`usage-coverage ${data.coverage.complete ? 'complete' : ''}`}><div><strong>{interpolate('Token 报告覆盖率 {coverage}%', { coverage })}</strong><span>{t(data.coverage.complete ? '供应商已为全部调用返回精确用量' : '未报告的调用保持未知，不计为 0 或估算值')}</span></div><div className="coverage-track"><i style={{ width: `${coverage}%` }} /></div></section>
        <div className="usage-columns">
          ...
          <section><h3>{t('模型明细')}</h3>{data.models.length ? <div className="usage-table">{data.models.map((item) => <div key={`${item.provider}:${item.model}`}><span><strong>{item.model}</strong><small>{item.provider}</small></span><span>{format(item.tokens.total)} tokens<small>{interpolate('{reported}/{total} 已报告', { reported: item.reported_invocations, total: item.invocations })}</small></span></div>)}</div> : <p className="usage-muted">{t('暂无模型调用')}</p>}</section>
