...
const english: Record<string, string> = {
  ...
  '默认': 'Default', '执行模式': 'Execution mode', '只规划任务，不调用工具或执行命令': 'Plan the task without calling tools or running commands', '自动执行低风险操作，高风险权限需要确认': 'Run low-risk actions automatically and confirm high-risk permissions', '自动执行所有命令和工具，不再请求确认': 'Run all commands and tools automatically without confirmation',
  '启用 ByPass 模式？': 'Enable ByPass mode?', 'ByPass 将允许 Agent 自动执行所有命令和工具，包括可能修改文件、访问网络或影响外部系统的高风险操作。': 'ByPass allows the agent to run every command and tool automatically, including high-risk actions that may modify files, access the network, or affect external systems.', '仅在你信任当前任务和运行环境时启用。': 'Enable this only when you trust the current task and runtime environment.', '取消': 'Cancel', '确认启用 ByPass': 'Enable ByPass',
