...
const english: Record<string, string> = {
  ...
  '了解对话策略': 'About chat strategies', '对话策略说明': 'Chat strategy guide', '关闭': 'Off',
  ...
  '思考过程': 'Reasoning process', '实时更新': 'Live', '进行中': 'In progress', '失败': 'Failed', '已完成': 'Completed', '正在理解任务并制定计划': 'Understanding the task and planning', '正在执行计划': 'Executing the plan', '正在分析下一步': 'Analyzing the next step', '正在组织回答': 'Composing the answer', '正在验证结果': 'Verifying the result', '正在处理': 'Working', '{steps} 个步骤': '{steps} steps', '{steps} 个步骤 · {tools} 次工具调用': '{steps} steps · {tools} tool calls', '思考': 'Reasoning', '验证': 'Verification', '数据与证据 · {count}': 'Data & evidence · {count}', '关联来源': 'Related source', '来源 · {count}': 'Sources · {count}', '运行工件': 'Run artifacts', '关联输出': 'Related outputs', '其他输出': 'Other outputs', '其他输出 · {count}': 'Other outputs · {count}', '{count} 个输出 · 查看输出': '{count} outputs · View outputs', '查看上方已展示的输出': 'View the output shown above', '预览加载失败': 'Preview failed to load', '隔离预览': 'Isolated preview',
