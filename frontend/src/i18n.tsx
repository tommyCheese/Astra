...
const english: Record<string, string> = {
  ...
  '次工具': 'tool calls', '当前强度可调整范围：{min}–{max} 次': 'Adjustable range for this effort: {min}–{max}', '允许 0–5 次工具调用，简单任务更快；启用反思时，提供轻量反思能力。': 'Allows 0–5 tool calls for faster simple tasks; when reflection is enabled, provides lightweight reflection capability.', '允许 6–15 次工具调用，兼顾速度与检查深度；启用反思时，提供基本的反思能力。': 'Allows 6–15 tool calls, balancing speed and depth; when reflection is enabled, provides basic reflection capability.', '允许 16–50 次工具调用，为复杂任务提供更多执行预算；启用反思时，允许更深层的反思能力。': 'Allows 16–50 tool calls for complex tasks; when reflection is enabled, allows deeper reflection capability.',
