...
const english: Record<string, string> = {
  ...
  '思考过程': 'Reasoning process', '{steps} 个步骤 · {tools} 次工具调用': '{steps} steps · {tools} tool calls', '思考': 'Reasoning', '验证': 'Verification', '来源 · {count}': 'Sources · {count}', '运行工件': 'Run artifacts', '关联输出': 'Related outputs', '其他输出': 'Other outputs', '其他输出 · {count}': 'Other outputs · {count}', '{count} 个输出 · 查看输出': '{count} outputs · View outputs', '查看上方已展示的输出': 'View the output shown above', '预览加载失败': 'Preview failed to load', '隔离预览': 'Isolated preview',
