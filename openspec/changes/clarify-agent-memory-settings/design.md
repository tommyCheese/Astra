...
2. **Memory 页面使用三个产品任务。** “记忆设置”管理强制运行策略；“已保存的记忆”显示内容、范围、状态、来源和撤销，并在同一详情中用默认折叠的高级区域披露召回、生命周期、版本和原始元数据；“整理与合并”显示 AutoDream。审计不是第二份 Memory 数据集，因此不使用独立一级入口或重复挂载 MemoryWorkbench。
...
- [高级审计信息让普通详情过重] → 保留在同一 Memory 详情中，但按审计分组默认折叠；用户需要诊断时再展开。
