...
聊天右侧使用紧凑概览；扩大窗格后使用半屏工作台。Plan 确认卡和终态审计不再维护各自的节点渲染逻辑。
...
节点默认按依赖 rank、`index`、`node_key` 稳定排序。图谱提供缩放、平移、定位中心和聚焦当前节点；“定位中心”只将 DAG 的几何中心移回画布中心并保留当前缩放比例，不再复用会改变缩放比例的 fit view。界面不开放节点拖动。移动端使用可滚动、可收起的图谱概览，不再重复显示线性节点列表。
...
节点同时使用文字、图标、边框和线型；边有方向箭头和依赖语义。工作台直接让节点按确定性层级顺序进入键盘焦点，并由节点自身向屏幕阅读器提供标题、状态、依赖和版本上下文。节点选择统一打开检查器，不再维护容易与图谱状态分叉的同步结构化列表。

`prefers-reduced-motion` 下关闭节点脉冲、边流动等持续动画。图谱原生节点始终可通过键盘访问，节点详情不依赖额外的线性列表或全屏层。
...
宽屏采用不遮挡对话的右侧双栏布局，窄屏降级为可收起的覆盖窗格。紧凑窗格展示图谱概览和显式的放大、缩小、定位中心控件；节点检查器在非紧凑工作台中按可用空间展示。计划确认、调整和取消仍留在确认卡中，避免把执行授权与图谱视口操作混为一谈。

右侧窗格提供容器级“扩大/恢复”操作。扩大后桌面内容区使用等宽双栏，图谱退出 compact 模式并将新增空间优先留给完整画布；不再显示信息价值有限且挤占画布的缩略图，也不再提供重复的图内全屏入口。该状态只改变界面布局，区别于 React Flow 的图内缩放、平移和保持缩放比例的定位中心。低于双栏断点时扩大态改为更宽的覆盖窗格，不强行压缩对话。
...
图谱保留 Astra 的薄荷绿主色、现有明暗主题和红/蓝/灰状态语义，但将视觉层级统一为三层：半透明工作台外壳、带低对比空间光和细网格的画布、带状态光带与高光边缘的节点卡片。依赖边使用细腻的双层描边、柔和阴影和清晰箭头，已完成路径、活动路径与阻塞路径仍由语义类名决定。

模糊只作用于工作台、节点和控件自身的 backdrop，不对大面积文本应用滤镜；动画只用于 running 状态的低频呼吸光，并在 reduced-motion 下完全关闭。高对比模式继续回退到实线、虚线和双线边框，确保精美效果不会降低可访问性。

## Risks / Trade-offs

- [DAG 在聊天宽度内过于密集] → 聊天只提供 fit-to-view 概览和当前路径，完整交互进入宽屏工作台，节点详情通过直接选择图中节点查看。
- [半屏窗格在中小视口压缩对话] → 只在宽屏使用等宽双栏，低于断点时改为可恢复的宽覆盖窗格。
...
- [新增图形依赖影响包体和维护] → 锁定最小依赖、按需加载图谱工作台、用纯数据 adapter 隔离库 API，并设置 bundle 检查。
