...
- [x] 9.1 明确新 Run 仅支持 adaptive 与 plan-first，并为历史 direct Run 保留只读兼容
- [x] 9.2 限制新建 Run 和偏好 API 的规划策略类型，将标准模式默认策略切换为 adaptive
- [x] 9.3 迁移持久化 direct 偏好，移除前端直接模式入口并更新帮助文案
- [x] 9.4 增加新请求拒绝 direct、旧快照兼容和偏好迁移测试，并运行后端、前端与 OpenSpec 校验
