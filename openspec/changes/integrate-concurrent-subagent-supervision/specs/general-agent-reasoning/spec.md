...
### Requirement: Trusted 根 Agent 使用一等并发委派决策
系统 SHALL 允许 eligible trusted 根控制器产生类型化 `delegate_tasks` 决策，其中包含有界 DelegationRequest 集合和一个 Join 规范，并 MUST NOT 将 Agent 委派表示为可绕过治理的普通第三方 Tool 调用。

#### Scenario: 控制器识别独立并行工作
- **WHEN** trusted 根控制器识别出两个相互独立、预期收益为正且符合策略的子任务
- **THEN** 控制器可在一个决策中提交两个完整 DelegationRequest 和 Join policy
- **THEN** 运行时在执行前验证目标、成功标准、范围、输入、输出 schema、能力、预算和去重信息

#### Scenario: Standard 控制器尝试委派
- **WHEN** standard Run 的控制器产生委派决策
