...
- `frontend/src/App.tsx` now selects `AgUiChatPage` through `VITE_AG_UI_ENABLED`, and the new page exercises the transport/store, Activities, interrupts, explicit cancellation and transport close. It is a deliberately small replacement surface rather than a parity-preserving migration of the established conversation shell, so visual/feature parity and browser verification remain open.
- Both backend and frontend currently default the AG-UI path to enabled. That is ahead of this proposal's staged rollout gates and must be treated as review debt: acceptance must either restore default-off until 12.x gates pass or explicitly revise and justify the rollout contract.
