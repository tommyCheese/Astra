...
前端事件缓冲在每个动画帧内进一步合并连续的同 stream `model_thinking.delta`：每个原始事件仍独立推进事件 ID、Run/Agent 游标和缺口检测，但只对过程条目执行一次正文拼接。模型思考组件不直接把网络累计正文作为当前帧内容，而维护独立的可见缓冲，按积压量动态提高每帧字符数；完成、减少动态效果或非追加更新时立即对齐。自动滚动跟随可见缓冲，并在绘制后通过单个动画帧回调合并 `scrollHeight` 测量，避免布局读取阻塞文本提交。
