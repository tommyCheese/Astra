...
- [x] 4.3 定义 `SandboxExecutor` 的 prepare、start、wait、terminate、collect 和 cleanup 协议
...
- [ ] 5.7 增加 Docker 可选集成测试，覆盖断网、只读文件系统、非 root、资源耗尽、终止和容器清理
...
- [ ] 6.6 增加 image contract tests，验证版本、字体、无网络、只读环境、输出目录和最小示例渲染
...
- [ ] 7.9 增加非法源码、超限数据、backend 选择、中文字体、空文件、损坏文件和可复现性测试
...
- [x] 9.1 增加 chart capability、executor、Docker/gVisor runtime、image digest、Artifact Store、资源配额和 retention 配置
...
- [x] 9.5 编写本地 Docker 启动、runtime image 构建和 Linux gVisor 部署文档
- [x] 9.6 编写威胁模型、运行时升级、image digest 发布、漏洞响应、Artifact 清理和事故处置文档

## 10. 端到端验证与迁移收尾

- [x] 10.1 增加 mock executor 端到端测试，覆盖 Agent 选择 `chart.render`、Job 生命周期、Artifact 回传和最终答案展示
- [ ] 10.2 增加 Docker 端到端测试，分别验证 Matplotlib、Seaborn 和 ECharts 的静态输出
