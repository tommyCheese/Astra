...
- [x] 2.6 Route ToolSpec attenuation, execution modes, Grants, Permission Bundles, protected resources, and data-flow egress through one Permission Engine invocation entry
...
- [x] 7.5 Feed ToolCall-level Workspace changes back into Bash observations and use them to stop completed one-step file tasks
...
- [x] 8.6 Redesign the permission center around human-readable active permissions, files, safety activity, and progressively disclosed technical details
- [x] 8.7 Simplify approval cards to action, user-visible resource, practical risk, and friendly scope choices only
- [x] 8.8 Render each ToolCall completion state once in the reasoning timeline
...
- [x] 10.1 Make Bash effect analysis fail closed for path-qualified executables, process substitution, ambiguous options, and multi-target mutations
- [x] 10.2 Protect control-plane Workspace paths in both permission matching and Sandbox mount enforcement
- [x] 10.3 Consume every lease used by a multi-effect invocation atomically
- [x] 10.4 Canonicalize sensitive data labels and enforce both accumulated and invocation-local labels during egress decisions
- [x] 10.5 Require authenticated Permission Bundles and enforce their runtime, data-label, credential, network, and output constraints
- [x] 10.6 Route credential issuance and delegation authorization through explicit unified Permission Engine decisions
- [x] 10.7 Reject remote API clients by default for the local single-user deployment model
- [x] 10.8 Add regression coverage for the discovered analyzer, protected-resource, lease, data-flow, Bundle, non-tool authorization, and API-boundary failures
- [x] 10.9 Run backend, frontend, OpenSpec, and focused security validation suites
