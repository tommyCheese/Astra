...
- **WHEN** 用户提交有效的五字段 cron 表达式、IANA 时区、目标和权限包
...
- **THEN** 系统创建全局可管理、以当前会话为执行目标的持久化任务，并返回标识符与下一触发时间

### Requirement: 会话中创建的定时任务具有全局控制面
系统 SHALL 将从任意会话创建的定时任务纳入同一个工作区清单，同时保留创建时选择的目标会话用于执行上下文与审计。

#### Scenario: 跨会话管理任务
- **WHEN** 用户在会话 A 创建任务后从会话 B 查询、暂停或手动运行该任务
- **THEN** 系统允许该操作，且任务执行目标仍为会话 A，除非用户显式修改目标
