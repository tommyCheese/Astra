...
JSON、CSV、TSV、Excel 与 Parquet 等明确数据格式归类为结构化数据制品；图片在页面内预览，HTML 使用无同源权限的 sandbox iframe 隔离预览。成功的外部写入或控制面工具调用形成操作回执，只从持久化审计中提取工具名、状态、目标、对象 ID 和 HTTP(S) 链接等白名单字段。原始输入、凭据、完整工具输出、只读调试日志和 stdout 不自动进入制品，避免把噪音或敏感内容扩大为用户可见交付物。

### 10. 资料库与已安排任务复用同一制品目录

资料库和已安排任务不得各自拼装一套制品。后端使用同一目录投影生成稳定制品 ID、来源字段和内容地址；已安排任务详情只是按 `job_id` 筛选该目录，资料库则展示全局集合。因此同一个结果、文件版本、结构化数据或操作回执在两处具有完全一致的引用。

工作空间仍是目标对话唯一且持续复用的工作空间。每次 Run 对已验证的可交付文件产生不可变 Artifact 快照，同时保留工作空间中的当前文件用于后续编辑；目录优先展示快照，并仅在旧数据没有快照且当前 checksum 仍与该次变更一致时回退到当前文件。这样同一路径被后续 Run 修改或删除时，历史任务制品不会指向错误内容，也不会把工作空间中无关文件归入某次自动化运行。
