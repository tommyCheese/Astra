...
### Requirement: 仅规划执行行为
**Reason**: 仅规划与可信执行的产品语义冲突，并产生不执行计划、延迟激活和特殊权限分支。
**Migration**: 取消包含该行为的非终态 Run；升级后的新 Run 必须选择快速响应或会实际执行的可信模式。

### Requirement: 自适应初始规划策略
**Reason**: 用户不应在可信执行中选择初始计划是否完整；可信模式固定在行动前创建完整 DAG。
**Migration**: 删除偏好和请求字段；所有 trusted Profile 由迁移统一为固定的执行前完整规划语义。
