...
ToolCall 完成后到下一轮 `selecting_action` 开始前，Agent 仍可能在标准化观察、评估成功条件、写入记忆或执行反思。前端在 `tool_call.completed` 后立即于当前决策分组末尾投影“正在评估执行结果”过渡条目，同时将父决策锚点标记完成。该过渡条目是此时唯一的运行中加载窗格；下一阶段或 Run 终态事件会自动结束它。过渡状态由已有持久化事件确定性重放，不新增仅存在于计时器中的伪进度。
