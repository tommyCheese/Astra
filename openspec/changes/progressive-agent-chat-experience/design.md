...
面板标题直接映射 Run 生命周期：只要 `ProcessStreamState.active` 为真就显示“思考中”，不依赖内部时间线是否恰好存在 `running` 条目；进入任一终态后显示“思考完成”。“思考中”右侧使用三个小圆点依次呼吸来表达持续活动，不让整个外层面板闪动；终态移除圆点，`prefers-reduced-motion` 下保留静态圆点。这样即使两个运行时事件之间存在短暂空档，折叠标题和展开面板顶部也持续提供明确的活动反馈。
...
`selecting_action` 不再和普通阶段、推理摘要及 ToolCall 平铺展示。前端使用其 `turn_index` 生成稳定决策分组 ID，把同轮 `reasoning_summary`、工具调用和反思投影为子条目；子条目整体缩进，并通过从父节点延伸的纵向归属线与短连接线表达层级。没有 `turn_index` 的实时 ToolCall 归属于事件顺序中最近的决策分组，终态则通过 AgentTurn 的 `tool_call_id` 重建确定性归属。

运行态 reducer 与终态 RunView 必须产生相同分组。决策分组仍处于运行态时，加载窗格只作用于父锚点，子条目保留透明背景和自己的状态文案，避免同一轮出现多个竞争的加载块。规划、回答组织和验证等非决策阶段继续作为顶层条目。

ToolCall 完成后到下一轮 `selecting_action` 开始前，Agent 仍可能在标准化观察、评估成功条件、写入记忆或执行反思。前端在 `tool_call.completed` 后立即于当前决策分组末尾投影“正在评估执行结果”过渡条目，同时将父决策锚点标记完成。该过渡条目是此时唯一的运行中加载窗格；下一阶段到达时直接移除过渡条目并展示新的运行阶段，Run 进入终态时也移除该临时条目，不把“正在评估执行结果 / 已完成”的矛盾状态保留在历史中。聊天仍处于自动跟随底部时，当前运行条目变化会触发滚动跟随；用户主动上滚后则不抢占位置。过渡状态由已有持久化事件确定性重放，不新增仅存在于计时器中的伪进度。
