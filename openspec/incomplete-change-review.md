...
Baseline: 2026-08-17. This is an audit index for review; it does not authorize implementation.
...
| `adopt-ag-ui-protocol-adapter` | 72/76 | backend projection and development preview exist; established-chat parity and rollout are incomplete | transport/store integration, browser parity/reconnect/accessibility, development rollout evidence, gated default selection |
...
1. Complete AG-UI established-chat transport/store integration before any default-selection work; backend and frontend flags are correctly default-off.
...
3. Use the same browser session to collect AG-UI parity, reconnect, accessibility and native-rollback evidence where practical.
4. Review compaction's remaining safety/evaluation work. Hook integration with compaction must wait for these boundary semantics, but the Hook proposal itself can be reviewed now.
5. Review the Hook proposal as a contract-only change. Implementation remains explicitly deferred until the event catalog, synchronous admission authority and staged handler rollout are approved.
