...
async def test_docker_provider_hardens_and_always_removes(tmp_path):
    ...
    await SandboxSupervisor(provider).run(SandboxRequest("astra-data-viz:0.1.0", ["/opt/astra/bin/render"], input_dir, output_dir))
...
async def test_docker_provider_removes_container_on_timeout(tmp_path):
    ...
    try:
        await SandboxSupervisor(provider).run(SandboxRequest("image", ["render"], tmp_path, tmp_path / "out", wall_time_seconds=0))
    except Exception:
        pass
    assert any(call[:2] == ("rm", "--force") for call in provider.calls)
