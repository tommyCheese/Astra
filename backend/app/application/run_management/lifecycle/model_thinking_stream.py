...
class ModelThinkingEventWriter:
    ...
    _run_id: str
    _streams: dict[str, _ThinkingStreamState] = field(default_factory=dict)
    _run_char_count: int = 0
    ...
    def _metadata(event: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "content_level": (
                event.get("content_level") if event.get("content_level") in {"reasoning", "summary"} else "unavailable"
            ),
        }
    ...
    async def _start(self, stream_id: str, event: dict[str, Any]) -> None:
        ...
        await self._repository.add_event(self._run_id, "model_thinking.started", metadata)
