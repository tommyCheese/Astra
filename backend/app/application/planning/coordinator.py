...
class RunCoordinator:
    ...
    async def _replay_recorded_results(
        ...
        async with self.session_factory() as session:
            ...
            recorded = [NodeExecutionResult.model_validate(execution.result) for execution in result.scalars().all()]
        completed: list[str] = []
        failed: list[str] = []
        for item in recorded:
            ...
            if retried:
                continue
            (completed if item.status == NodeExecutionStatus.completed else failed).append(item.execution_id)
    ...
    async def _merge_result(
        ...
        async with self.session_factory() as session:
            ...
            should_retry = (
                result.status == NodeExecutionStatus.failed and result.retryable and result.attempt <= self.max_safe_retries
            )
            if should_retry:
                await self._schedule_retry(session, execution_repository, execution, node, run, result)
                return True
    ...
    async def _schedule_retry(self, session, execution_repository, execution, node, run, result) -> None:
        await execution_repository.settle_budgets(execution.id, consumed=result.budget_consumed, status="settled")
...
def _terminal_outcome(status):
    ...
    node_status = PlanNodeStatus.blocked.value if status == NodeExecutionStatus.blocked else PlanNodeStatus.failed.value
...
async def _build_context(
    ...
    *,
    ...
    predecessor_ids = {edge.predecessor_id for edge in dependencies if edge.successor_id == execution.plan_node_id}
    evidence_result = await session.execute(select(PlanNodeRecord.evidence_refs).where(PlanNodeRecord.id.in_(predecessor_ids)))
    evidence = tuple(dict.fromkeys(ref for refs in evidence_result.scalars().all() for ref in (refs or [])))
    state = run.agent_state or {}
    execution = await NodeExecutionRepository(session).require(execution.id)
    skill_snapshot = await session.scalar(select(RunSkillSnapshotRecord).where(RunSkillSnapshotRecord.run_id == run.id))
    ...
    return NodeContextSnapshot(
        ...
        reserved_budgets={reservation.budget_kind: reservation.reserved for reservation in execution.budget_reservations},
...
def _skill_context(node, skill_snapshot):
    ...
    catalog = tuple(
        {key: item[key] for key in ("qualified_identity", "name", "description", "revision_id", "digest")}
        for item in (skill_snapshot.catalog if skill_snapshot else [])
        if item["qualified_identity"] in required_ids
    )
    active_ids = {item["qualified_identity"] for item in (skill_snapshot.activations if skill_snapshot else [])}
...
def _retry_safe(node, parallel_safe_capabilities) -> bool:
    if not node.required_capabilities:
        return True
    return parallel_safe_capabilities is not None and (set(node.required_capabilities) <= parallel_safe_capabilities)


def _merge_run_state(run: RunRecord, result: NodeExecutionResult) -> None:
    state = dict(run.agent_state or {})
    state["active_executions"] = [
        item for item in state.get("active_executions", []) if item.get("execution_id") != result.execution_id
    ]
