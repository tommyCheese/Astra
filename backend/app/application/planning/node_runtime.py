...
from app.application.planning.coordinator import NodeContextSnapshot
...
async def prepare_parallel_node_runtime(
    settings: AstraRuntimeSettings, repository: RunUnitOfWork, context: NodeContextSnapshot
) -> ParallelNodeRuntime:
    ...
    observations = [
        observation_from_call(call) for call in run.tool_calls if call.id in dependency_refs and call.status == "succeeded"
    ]
    goal = str(
        context.task_contract.get("original_goal") or run.model_policy.get("conversation_goal") or context.node["intent"]
    )
...
def observation_from_call(call: ToolCallRecord) -> dict[str, Any]:
