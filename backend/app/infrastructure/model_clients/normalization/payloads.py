...
def normalize_contract_payload(payload: dict[str, Any], goal: str) -> dict[str, Any]:
    ...
    normalized["verification_requirements"] = _normalize_requirements(normalized.get("verification_requirements"))
    _normalize_ambiguity(normalized)
    return normalized


def _goal_mismatch(reported_goal: str, expected_goal: str) -> bool:
    return bool(reported_goal and normalize_goal_text(reported_goal) != normalize_goal_text(expected_goal))
...
def _findings(value: object) -> list[dict[str, Any]]:
    ...
    for raw in _items(value):
        ...
        item["artifact_ids"] = (
            [identifier for identifier in artifact_ids if isinstance(identifier, str)] if isinstance(artifact_ids, list) else []
        )
...
def _memory_key(payload: dict[str, Any], scope: str, kind: str, content: str) -> str:
    ...
    if proposed and len(proposed) <= 240 and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:/-]*", proposed):
        return proposed
...
def _expected_outcome(value: object) -> dict[str, Any]:
    ...
    return {
        ...
        "success_condition": str(expected.get("success_condition") or "step completed with accepted evidence"),
...
def _normalize_patch(patch: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["invalidated_assumption_ids"] = _list_or_empty(patch.get("invalidated_assumption_ids"))
    normalized["criterion_updates"] = patch.get("criterion_updates") if isinstance(patch.get("criterion_updates"), dict) else {}
    ...
    normalized["added_verification_requirements"] = _requirements(patch.get("added_verification_requirements"))
    return normalized
...
def _fact(raw: dict[str, Any], index: int, statement: str) -> dict[str, Any]:
    ...
    return {
        ...
        "statement": statement,
        "provenance": provenance if isinstance(provenance, dict) else {"source": "model_reflection"},
