...
async def get_usage_summary(
    ...
    if scope == "task" and not task_id:
        raise ValidationError("TASK_ID_REQUIRED", "查询当前对话用量时必须提供 task_id。")
    if scope == "run" and not run_id:
        raise ValidationError("RUN_ID_REQUIRED", "查询单次运行用量时必须提供 run_id。")
    if from_time and to_time and from_time >= to_time:
        raise ValidationError("USAGE_RANGE_INVALID", "用量查询的开始时间必须早于结束时间。")
