...
def evaluate_manifest(
    ...
    *,
    ...
    if required_thresholds is not None:
        issues.extend(_threshold_policy_issues(thresholds, required_thresholds))

    success_delta = manifest.candidate.success_rate - manifest.baseline.success_rate
    success_regression = max(0.0, -success_delta)
    if success_regression > thresholds.max_success_rate_regression:
        issues.append(
            EvaluationIssue(
                code="evaluation.success_rate_regression",
                message="Candidate task success exceeds the allowed regression.",
                metric="success_rate",
                baseline=manifest.baseline.success_rate,
                candidate=manifest.candidate.success_rate,
                limit=thresholds.max_success_rate_regression,
            )
        )

    cost_ratio = _increase_ratio(manifest.baseline.mean_cost, manifest.candidate.mean_cost)
    if cost_ratio > thresholds.max_cost_increase_ratio:
        issues.append(
            EvaluationIssue(
                code="evaluation.cost_regression",
                message="Candidate cost exceeds the allowed increase.",
                metric="mean_cost",
                baseline=manifest.baseline.mean_cost,
                candidate=manifest.candidate.mean_cost,
                limit=thresholds.max_cost_increase_ratio,
            )
        )
    ...
    if latency_ratio > thresholds.max_latency_increase_ratio:
        issues.append(
            EvaluationIssue(
                code="evaluation.latency_regression",
                message="Candidate latency exceeds the allowed increase.",
                metric="mean_latency_ms",
                baseline=manifest.baseline.mean_latency_ms,
                candidate=manifest.candidate.mean_latency_ms,
                limit=thresholds.max_latency_increase_ratio,
            )
        )
    ...
    for metric in manifest.safety_metrics:
        if metric.regressed:
            issues.append(
                EvaluationIssue(
                    code="evaluation.safety_regression",
                    message="A protected safety metric regressed.",
                    metric=metric.name,
...
def transition_candidate_state(
    ...
    *,
    ...
    if target in {
        ...
        evaluation_digest = evaluation_manifest.digest
...
def _threshold_policy_issues(
    ...
    comparisons = (
        (
            "minimum_sample_size",
            actual.minimum_sample_size,
            required.minimum_sample_size,
            actual.minimum_sample_size < required.minimum_sample_size,
        ),
        (
            "minimum_held_out_cases",
            actual.minimum_held_out_cases,
            required.minimum_held_out_cases,
            actual.minimum_held_out_cases < required.minimum_held_out_cases,
        ),
        (
            "max_success_rate_regression",
            actual.max_success_rate_regression,
            required.max_success_rate_regression,
            actual.max_success_rate_regression > required.max_success_rate_regression,
        ),
        (
            "max_cost_increase_ratio",
            actual.max_cost_increase_ratio,
            required.max_cost_increase_ratio,
            actual.max_cost_increase_ratio > required.max_cost_increase_ratio,
        ),
        (
            "max_latency_increase_ratio",
            actual.max_latency_increase_ratio,
            required.max_latency_increase_ratio,
            actual.max_latency_increase_ratio > required.max_latency_increase_ratio,
        ),
    )
    for name, candidate_value, required_value, too_permissive in comparisons:
        if too_permissive:
            issues.append(
                EvaluationIssue(
...
def _increase_ratio(baseline: float, candidate: float) -> float:
    if baseline == 0:
        # A positive candidate value over a zero baseline is an unbounded
        # increase. Keep the decision JSON-safe while guaranteeing it fails
        # every finite governed threshold.
