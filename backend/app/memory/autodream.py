    ...
    async def prepare_job(
        ...
        *,
        ...
        try:
            ...
            usage["budgets"] = {
                "max_records": (
                    self.settings.agent_memory_autodream_max_records_per_job
                ),
                "max_model_calls": (
                    self.settings.agent_memory_autodream_max_model_calls
                ),
            }
            profile_snapshot = autodream_profile_snapshot(load_agent_profile())
            if (
                len(records)
                < self.settings.agent_memory_autodream_min_candidates
            ):
                        ...
                        "issues": [
                            {
                                ...
                                "detail": (
                                    "Eligible input count is below the configured "
                                    "minimum"
                                ),
                            }
                        ],
            ...
            manifest = ConsolidationInputManifest.build(
                ...
                namespace_id=claimed.namespace_id,
                items=(
                    FrozenMemoryInput.from_record(record)
                    ...
                    "issues": [
                        {
                            ...
                            "detail": (
                                "The bounded input contains no deterministic "
                                "duplicate group"
                            ),
                        }
                    ],
...
class AutoDreamService:
    ...
    async def startup(self) -> None:
        ...
        async with self.session_factory() as session:
            recovered = await MemoryConsolidationRepository(
                session
            ).recover_expired()
        if recovered:
    ...
    async def run_once(self) -> dict[str, Any]:
        ...
        processed: list[str] = []
        async with self.session_factory() as session:
            queued = await MemoryConsolidationRepository(
                session
            ).queued_job_ids(
        ...
        async with self.session_factory() as session:
            ...
            for namespace_type, namespace_id, _count in namespaces:
                ...
                if not cooldown_elapsed(
                    ...
                    now=utc_now(),
                    cooldown_seconds=(
                        self.settings.agent_memory_autodream_cooldown_seconds
                    ),
                ):
                    continue
