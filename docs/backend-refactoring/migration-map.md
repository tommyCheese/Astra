...
| `app/runner/agent_loop.py` | 整个 root Agent 状态机及大量横切关注点 | agent-runtime | `app/agent_runtime/orchestrator.py`、`phases/*`、`context/*` | 所有执行入口切换，旧 loop 删除，模块/函数低于硬限制 |
...
| `app/runner/planning.py` | plan validation、patch、ready-node scheduling | planning | `app/runner/planning.py`、`plan_scheduler.py`、`plan_errors.py` | 已完成：调度和错误消费者使用真实所有者模块，旧跨模块 re-export 已删除 |
