...
前端“记忆”设置按用户任务分成“记忆设置”“已保存的记忆”和“整理与合并”。“已保存的记忆”默认展示内容、范围、状态与来源；召回评分、排除原因、版本、生命周期和原始溯源数据在同一详情的“审计详情”中默认折叠，展开不会加载第二份 Memory 列表。AutoDream 是“整理与合并”的后台维护机制，不是一种 Memory，也不会修改 Agent Profile。
