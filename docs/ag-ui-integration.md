...
后端与第一方前端在门禁通过后默认启用 AG-UI。前端通过 transport 选择器在 AG-UI 与原生 transport 之间切换；紧急回滚时设置后端 `AG_UI_ENABLED=false`、前端 `VITE_AG_UI_ENABLED=false`，原生 `/api/runs/stream` 和事件端点不会删除。
...
开发 cohort 已在上述自动化门禁全部通过后开启；安全、恢复、延迟、可访问性和可见结果一致性门禁均记录在本变更测试中。第一方现默认选择 AG-UI，任一部署可用双端开关立即回滚到原生 transport。原生 transport 的删除必须另开 breaking change。
